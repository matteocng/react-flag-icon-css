...
export const ThrowNoJsonLoaderError = () => {
  if (process.env.NODE_ENV !== 'production') {
    // UglifyJS strips this code out in production.
    ...
    const isWebpackOne =
      webpackSignatures[0] !== 'undefined' ||
      webpackSignatures[1] !== 'undefined'

    const message = 'react-flag-icon-css needs to import json files'
    const suggestion = isWebpackOne
      ? ' update to Webpack 2 to load json out of the box or add ' +
